...
fn blocks_matching_qualified_pane_id() {
    std::env::set_var("ZELLIJ_PANE_ID", "8");
    let result = check_self_write("terminal_8", false);
...
fn blocks_bare_integer_matching_pane_id() {
    std::env::set_var("ZELLIJ_PANE_ID", "8");
    let result = check_self_write("8", false);
...
fn allows_different_pane() {
    std::env::set_var("ZELLIJ_PANE_ID", "8");
    let result = check_self_write("terminal_3", false);
    assert!(result.is_ok());
    std::env::remove_var("ZELLIJ_PANE_ID");
}
...
fn bypassed_with_no_guard() {
    std::env::set_var("ZELLIJ_PANE_ID", "8");
    let result = check_self_write("terminal_8", true);
    assert!(result.is_ok());
    std::env::remove_var("ZELLIJ_PANE_ID");
}
